...
resource "aws_subnet" "pri" {
  ...
  vpc_id                  = data.aws_vpc.vpclist.id
  #cidr_block              = each.value
  # cidr_block = cidrsubnet(data.aws_vpc.vpclist.cidr_block, 8, (index(keys(var.mod-prisub)), each.key))
  cidr_block = cidrsubnet(data.aws_vpc.vpclist.cidr_block, 8, (index(keys(var.prisub), each.key)))
