...
locals {
 # total_azs         = min(var.mod-az_count, length(data.aws_availability_zones.available.names))
  total_public_subnets  = var.mod-public_subnet_count * var.mod-az_count  # Total public subnets
  total_private_subnets = var.mod-private_subnet_count * var.mod-az_count # Total private subnets
   depends_on = [
